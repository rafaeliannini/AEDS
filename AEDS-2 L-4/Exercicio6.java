...
class TabelaHashDiretaComRehash {
    ...
    public TabelaHashDiretaComRehash() {
        tamTab = 21;
        tabela = new Pokemon[tamTab];
    }

    private int funcaoHash1(String nome) {
        int somaASCII = 0;
    ...
    private int funcaoHash2(String nome) {
        int somaASCII = 0;
        for (char c : nome.toCharArray()) {
            somaASCII += c;
        }
        return (somaASCII + 1) % tamTab;
    }

    public void inserir(Pokemon pokemon) {
        int pos = funcaoHash1(pokemon.getName());

        if (tabela[pos] == null) {
            tabela[pos] = pokemon;
        } else {
            int pos2 = funcaoHash2(pokemon.getName());
            if (tabela[pos2] == null) {
                tabela[pos2] = pokemon;
            }else{
                return;
    ...
    public boolean pesquisar(String nome) {
        ...
        int pos2 = funcaoHash2(nome);
        while (tabela[pos2] != null) {
            comparacoes++;
            if (tabela[pos2].getName().equalsIgnoreCase(nome)) {
...
public class Exercicio6 {
    public static void main(String[] args) {
        TabelaHashDiretaComRehash pokedex = new TabelaHashDiretaComRehash();
