...
void insertionSortPartial(Pokemon *array[], int n, int k, int *comparisons, int *movements) {
    if (k > n) k = n;
...
int main() {
    ...
    int comparisons = 0;
    int movements = 0;
    insertionSortPartial(foundList, foundCount, 10, &comparisons, &movements);

    int printCount = foundCount < 10 ? foundCount : 10;
    ...
    char filename[50];
    snprintf(filename, sizeof(filename), "%s_insertionSortP.txt", matricula);
