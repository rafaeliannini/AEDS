...
void bubbleSort(Pokemon *array[], int n, int *comparisons, int *movements) {
    bool swapped;
    for (int i = 0; i < n - 1; i++) {
        swapped = false;
...
int main() {
    ...
    int comparisons = 0;
    int movements = 0;
    bubbleSort(foundList, foundCount, &comparisons, &movements);
    ...
    char filename[50];
    snprintf(filename, sizeof(filename), "%s_bubblesort.txt", matricula);
