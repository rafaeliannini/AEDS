...
void radixSort1(Pokemon *array[], int n, int exp, int maxChar) {
    Pokemon *output[n];
    int count[maxChar + 1];
    ...
    for (int i = 0; i < n; i++) {
        int charAt = getCharAt(array[i]->abilities[exp / 256], exp % 256);
        count[charAt]++;
    }
...
int maxAbilityLen(Pokemon *array[], int n) {
    int maxLen = 0;
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < 6; j++) {
            int len = strlen(array[i]->abilities[j]);
            if (len > maxLen) {
                maxLen = len;
            }
        }
...
void radixSort(Pokemon *array[], int n, int *comparisons, int *movements) {
    int maxLen = maxAbilityLen(array, n);
    int maxChar = 255;

    for (int exp = maxLen * 256 - 1; exp >= 0; exp--) {
        (*comparisons) ++;
        (*movements) += 3;
        radixSort1(array, n, exp, maxChar);
    }

    for (int i = 1; i < n; i++) {
        (*comparisons) ++;
        if (strcmp(array[i - 1]->abilities[0], array[i]->abilities[0]) == 0) {
            (*comparisons) ++;
...
int main() {
    ...
    int comparisons = 0;
    int movements = 0;
    radixSort(foundList, foundCount, &comparisons, &movements);
    ...
    char filename[50];
    snprintf(filename, sizeof(filename), "%s_radixsort.txt", matricula);
