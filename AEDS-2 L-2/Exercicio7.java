...
public class Exercicio7 {
    ...
    private static void insertionSort(List<Pokemon> pokedexList) {
        ...
        for (int i = 1; i < n; i++) {
            ...
            while (j >= 0 && (pokedexList.get(j).getCaptureDate().isAfter(key.getCaptureDate()) || 
                (pokedexList.get(j).getCaptureDate().isEqual(key.getCaptureDate()) &&
                 pokedexList.get(j).getName().compareToIgnoreCase(key.getName()) > 0))) {
                comparacoes++;
