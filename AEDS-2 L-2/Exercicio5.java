...
public class Exercicio5 {
    ...
    private static void selectionSort(List<Pokemon> pokedexList) {
        int n = pokedexList.size();
        for (int i = 0; i < n - 1; i++) {
            int minIndex = i;
            for (int j = i + 1; j < n; j++) {
                if (pokedexList.get(j).getName().compareToIgnoreCase(pokedexList.get(minIndex).getName()) < 0) {
                    minIndex = j;
                    comparacoes ++;
                }
            }
            if (minIndex != i) {
                ...
                pokedexList.set(i, pokedexList.get(minIndex));
                pokedexList.set(minIndex, temp);
            }
        }
    }
}