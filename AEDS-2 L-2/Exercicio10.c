...
int partition(Pokemon *array[], int esq, int dir, int *comparisons, int *movements) {
    Pokemon *pivot = array[dir];
    int i = esq - 1;

    for (int j = esq; j < dir; j++) {
        (*comparisons)++;
        
        if (array[j]->generation < pivot->generation || 
            (array[j]->generation == pivot->generation && strcasecmp(array[j]->name, pivot->name) < 0)) {
            i++;
...
void quickSort(Pokemon *array[], int esq, int dir, int *comparisons, int *movements) {
    if (esq < dir) {
        int pi = partition(array, esq, dir, comparisons, movements);

        quickSort(array, esq, pi - 1, comparisons, movements);
        quickSort(array, pi + 1, dir, comparisons, movements);
    }
}
...
int main() {
    ...
    int comparisons = 0;
    int movements = 0;
    quickSort(foundList, 0, foundCount - 1, &comparisons, &movements);
    ...
    char filename[50];
    snprintf(filename, sizeof(filename), "%s_quicksort.txt", matricula);
