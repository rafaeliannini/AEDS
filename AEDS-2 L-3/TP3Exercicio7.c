...
void removerInicio(Lista *lista, int teste) {
...
int getMedia(Pokemon *pokemon){
    int media = pokemon->captureRate;
    return media;
}

void printarMedia(Lista *lista){
    int media = 0;
    for (Celula *i = lista->primeiro->prox; i != NULL; i = i->prox) {
        media += getMedia(&i->elemento);
    }
    media = (media + (lista->tamanho / 2)) / lista->tamanho;
    printf("Média: %d\n", media);
}
    ...
    while (1) {
        ...
        for (Celula *i = lista.primeiro->prox; i != NULL; i = i->prox) {
            if (strcmp(i->elemento.id, id) == 0) {
                if(foundList.tamanho == 5){
                    removerInicio(&foundList, 0);
                }
                inserirFim(&foundList, i->elemento);
                printarMedia(&foundList);
                found = true;
                break;
            }
        }
